...
nu <- max(best_weights_volatility$shape)
...
## Calculating the degrees of freedom
nu <- max(best_weights_sharpe$shape)
...
all_betas <- all_betas %>%
  ...
  dplyr::rename(weights_opt_vol = weights) %>%
  dplyr::mutate(weighted_beta_opt_vol = beta*weights_opt_vol) %>%
  base::merge(., best_weights_sharpe %>% select(ticker, weights), by.x="ticker", by.y="ticker", all.x=F, all.y=F) %>%
  dplyr::rename(weights_opt_sr = weights) %>%
  dplyr::mutate(weighted_beta_opt_sr = beta*weights_opt_sr)
...
beta_opt_vol <- sum(all_betas$weighted_beta_opt_vol)
...
```{r}
## Calculating the beta value for the portfolio
beta_opt_sr <- sum(all_betas$weighted_beta_opt_sr)
